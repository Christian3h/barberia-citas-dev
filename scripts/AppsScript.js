...
/**
 * Registra eventos de alerta crítica en la hoja 'SystemLogs'
 * Crea una trazabilidad permanente de intentos de citas duplicadas, borrados o purgas.
 */
function logSystemAlert(type, message, details) {
  try {
    ...
    if (!sheet) {
      ...
      sheet.getRange(1, 1, 1, 4).setFontWeight('bold').setBackground('#2d2d2d').setFontColor('#ffffff');
    }

    const timestamp = new Date().toISOString();
    const detailsStr = typeof details === 'object' ? JSON.stringify(details) : String(details || '');

    sheet.appendRow([timestamp, type, message, detailsStr]);
    log('WARN', 'dev_alert', `[${type}] ${message}`, details);
  } catch (e) {
    log('ERROR', 'dev_alert', 'Fallo al escribir en SystemLogs', { error: e.message });
  }
}
...
function parseDatetimeBogota(dateInput, timeInput, tzOffset) {
  ...
  try {
    ...
    // Si dateInput ya es un Date válido
    if (dateInput instanceof Date && !isNaN(dateInput.getTime())) {
      // Si no nos pasaron timeInput específico (o vino null/vacío), la fecha ya tiene hora (ej. datetime_iso en Sheets)
      if (timeInput === null || timeInput === undefined || timeInput === '') {
        return dateInput;
      }
    }

    // Si dateInput es un String ISO completo (ej. "2026-08-10T14:00:00.000Z")
    ...
    if (dateInput instanceof Date) {
      // Extraer componentes ajustados al timezone local para evitar drift de día
      const localMs = dateInput.getTime() + offset * 3600000;
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idCol]).trim() === idStr) {
      ...
      if (sheetName === 'Appointments') {
        logSystemAlert('APPOINTMENT_DELETED', `Cita eliminada de la hoja Appointments`, { id: idStr, row: i + 1 });
      }
      return { success: true, id: idStr };
    }
  }
...
/**
 * Convierte cualquier valor de celda de fecha en "YYYY-MM-DD" de forma consistente.
 */
function normalizeDateStr(val, tzOffset) {
  if (!val) return '';
  if (val instanceof Date) {
    return formatDateLocal(val, tzOffset);
  }
...
/**
 * Convierte cualquier valor de celda de hora en minutos transcurridos desde medianoche.
 */
...
/**
 * Verifica si existe conflicto/solapamiento de horario para un barbero en una fecha.
 * Retorna { conflict: true, reason: string } si existe colisión de rangos.
 */
function checkAppointmentConflict(barberId, date, time, durationMin, tzOffset) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName('Appointments');
  if (!sheet) return { conflict: false };

  const data = sheet.getDataRange().getValues();
  if (data.length <= 1) return { conflict: false };
  ...
  if (barberIdCol === -1 || dateCol === -1 || timeCol === -1 || statusCol === -1) {
    return { conflict: false };
  }

  const reqBarberId = String(barberId).trim();
  const reqDate     = normalizeDateStr(date, tzOffset);
  const reqStartMin = timeToMinutes(time);
  const reqDuration = parseInt(durationMin) || 30;

  if (reqStartMin === null) {
    return { conflict: true, reason: `Hora requerida inválida: ${time}` };
  }
  const reqEndMin = reqStartMin + reqDuration;

  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    const rowStatus = String(row[statusCol]).trim().toLowerCase();
    if (rowStatus !== 'scheduled') continue;

    const rowBarberId = String(row[barberIdCol]).trim();
    if (rowBarberId !== reqBarberId) continue;

    const rowDate = normalizeDateStr(row[dateCol], tzOffset);
    if (rowDate !== reqDate) continue;

    const rowStartMin = timeToMinutes(row[timeCol]);
    if (rowStartMin === null) continue;

    const rowDuration = durationCol !== -1 && parseInt(row[durationCol]) > 0 ? parseInt(row[durationCol]) : 30;
    const rowEndMin   = rowStartMin + rowDuration;

    // Verificación de solapamiento de rangos [reqStart, reqEnd) con [rowStart, rowEnd)
    if (reqStartMin < rowEndMin && reqEndMin > rowStartMin) {
      const rowTimeStr = normalizTimeForDisplay(row[timeCol]);
      log('WARN', 'checkConflict', 'Conflicto detectado', {
        req: `${reqDate} ${time} (${reqDuration}m)`,
        existing: `${rowDate} ${rowTimeStr} (${rowDuration}m), fila ${i + 1}`
      });
      return {
        conflict: true,
        reason: `Ya existe una cita en este horario (${rowTimeStr}, duración ${rowDuration} min)`
      };
    }
  }
 ...
 * - Verifica duplicados y solapamiento de rangos de tiempo
 ...
 *
 * @param {Object} data - { barber_id, customer_name, phone, email, service_name, date, time, duration_min, notes }
...
function createAppointment(data) {
  ...
  try {
    ...
    const durationMin = data.duration_min ? parseInt(data.duration_min) : serviceDuration;

    // --- Verificar duplicados / solapamiento con la función robusta ---
    ...
    if (conflictCheck.conflict) {
      logSystemAlert('DUPLICATE_PREVENTED', 'Intento de agendamiento duplicado o sobrepuesto bloqueado', {
      ...
      return { error: conflictCheck.reason || 'Ya existe una cita para ese barbero en esa fecha y hora' };
    }
...
function purgeOldAppointments() {
  ...
  const settings = settingsResult.success ? settingsResult.data : {};
  const tzOffset              = parseFloat(settings.timezone_offset)             || -5;
  const purgeAfterDaysRaw = parseInt(settings.purge_after_days);
  const purgeAfterDays = isNaN(purgeAfterDaysRaw) || purgeAfterDaysRaw < 1 ? 7 : purgeAfterDaysRaw;

  const purgeScheduledRaw = parseInt(settings.purge_scheduled_after_days);
  const purgeScheduledAfterDays = isNaN(purgeScheduledRaw) || purgeScheduledRaw < 1 ? 3 : purgeScheduledRaw;
  ...
  logSystemAlert('APPOINTMENTS_PURGED', `Purga automática completada: ${rowsToArchive.length} citas movidas a Archive`, {
...
const ALLOWED_SHEETS = ['Appointments', 'Users', 'Services', 'Unavailable', 'BlockedDays', 'Archive', 'SystemLogs'];
